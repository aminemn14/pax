...
---------------------------------------------------*/
  ...
  function CleanupBeforeAjax() {
    ...
    paxSlider.instances.forEach((slider) => slider.off());
    paxSlider.instances = [];
  }
    ...
    function initOnFirstLoad() {
      imagesLoaded("body", function () {
        ...
        setTimeout(function () {
          ...
          gsap.set($(".showcase-gallery .pax-slider .slide-inner"), {
            opacity: 0,
          });
          gsap.to($(".showcase-gallery .pax-slider .pax-slide .slide-inner"), {
            duration: 2,
            opacity: 1,
            delay: 0.3,
            ease: Power4.easeOut,
          ...
          gsap.set(
            $(
              "#filters-wrapper, .pax-pagination, .cp-button-prev, .cp-button-next, .progress-info, footer .link-text"
            ),
            { opacity: 0 }
          );
          gsap.to(
...
---------------------------------------------------*/
  ...
  function PageLoadActions() {
    ...
    // stories Page Navigation Load Events
    if (!isMobile()) {
      $("#stories-nav .next-ajax-link-stories").mouseenter(function (e) {
        var $this = $(this);
        $("#ball").append(
          '<p class="first">' +
            $this.data("firstline") +
        ...
        $("#ball").addClass("with-blur");
        $("#stories-nav .next-hero-title").addClass("hover-title");
      });

      $("#stories-nav .next-ajax-link-stories").mouseleave(function (e) {
        gsap.to("#ball", {
          duration: 0.2,
          borderWidth: "4px",
          scale: 0.5,
        ...
        $("#ball").removeClass("with-blur");
        $("#ball p").remove();
        $("#stories-nav .next-hero-title").removeClass("hover-title");
      });
    }

    if (!$("body").hasClass("disable-ajaxload")) {
      if ($(".next-ajax-link-stories").hasClass("auto-trigger")) {
        if (
          !(
            ...
            const ReachBottom = ScrollTrigger.create({
              ...
              onEnter: function (st) {
                ...
                $this.delay(500).queue(function () {
                  gsap.set(
                    $("#stories-nav.change-header, .next-hero-progress"),
                    { backgroundColor: "transparent" }
                  );
                  ...
                  var link = $this.find(".next-ajax-link-stories");
                  link.trigger("click");
                });
              },
      ...
      $(".next-ajax-link-stories").on("click", function () {
        $("body").addClass("load-stories-thumb");
        $("header").removeClass("white-header");
        $("#app").remove();
        $(".next-stories-image-wrapper").addClass("temporary").appendTo("body");
        ...
        if ($(".next-stories-caption").hasClass("height-title")) {
          const spanLetters = document.querySelectorAll(
            ".next-stories-caption .next-hero-title span"
          );
          const shuffledSpans = Array.from(spanLetters).sort(
            () => Math.random() - 0.5
          );
        ...
        gsap.set($("#stories-nav.change-header, .next-hero-progress"), {
          backgroundColor: "transparent",
        });
        ...
        gsap.to($(".next-stories-image"), {
          ...
          ease: Power2.easeInOut,
          onComplete: function () {
            $(".temporary .next-stories-image").addClass("visible");
          },
        });
      ...
      $(".next-ajax-link-stories").on("click", function () {
        $("body")
          .addClass("load-stories-thumb-with-title")
        ...
        $("#ball i").remove();
        gsap.to(
          $("#main-page-content, #hero, #hero-image-wrapper, #stories-nav"),
...
---------------------------------------------------*/
  ...
  function LazyLoad() {
    ...
    if ($("#hero").hasClass("has-image")) {
      if ($("body").hasClass("load-stories-thumb")) {
        if ($("#hero-caption").hasClass("height-title")) {
          const spanLetters = document.querySelectorAll(
            "#hero-caption .hero-title.caption-timeline span"
          );
      ...
      } else if ($("body").hasClass("load-stories-thumb-with-title")) {
    ...
    if ($(".load-stories-thumb").length > 0) {
      imagesLoaded("#hero-image-wrapper", function () {
        ...
        setTimeout(function () {
          ...
          gsap.to($(".next-stories-image-wrapper.temporary"), {
            duration: 0.1,
            opacity: 0,
            ease: Power2.easeOut,
            onComplete: function () {
              $(".next-stories-image-wrapper.temporary").remove();
    ...
    } else if ($(".load-stories-thumb-with-title").length > 0) {
      imagesLoaded("#hero-image-wrapper", function () {
        if (isMobile()) {
          $("#hero-image-wrapper")
            .find("video")
        ...
        setTimeout(function () {
          ...
          $("body")
            .removeClass("load-stories-thumb-with-title")
    ...
    setTimeout(function () {
      $("header").removeClass("white-header");
      $("body").removeClass(
        "load-stories-thumb load-stories-thumb-with-title load-next-page grid-open"
      );
      setTimeout(function () {
        imagesLoaded("body", function () {
...
---------------------------------------------------*/
  ...
  /*--------------------------------------------------
Function Showcase liste
---------------------------------------------------*/

  function ShowcaseListe() {
    if ($(".showcase-liste").length > 0) {
      var numberOfDivs = 13;
      ...
      if (isMobile()) {
        gsap.set($(".showcase-liste .slide-caption"), {
          y: 30,
          opacity: 0,
        });
      }
          ...
          scrollTrigger: {
            ...
            onEnter: function () {
              ...
              if ($("body").hasClass("show-loader")) {
                setTimeout(function () {
                  gsap.to(
                    $(".showcase-liste .img-mask.animated .curtain-row"),
                    {
                      duration: 0.7,
                      scaleY: 0,
                      opacity: 0,
      ...
      function filter() {
        var state = Flip.getState(".pax-item");
        var story = document.querySelectorAll(".pax-item");
        var startHeight = gsap.getProperty(".showcase-liste", "height");

        var filters = document.querySelectorAll(".filter-option.is_active");
        var parallaxItems = document.querySelectorAll(
          ".showcase-liste .pax-item.vertical-parallax .slide-inner"
        );

        var hasFilteredItems = false;

        if (filters.length) {
          story.forEach(function (stories) {
            gsap.set(stories, { display: "block" });
            stories.classList.remove("filtered");
          });
          filters.forEach(function (filter) {
            ...
            if (colorClass !== "") {
              story.forEach(function (stories) {
                if (!stories.classList.contains(colorClass)) {
                  gsap.set(stories, { display: "none" });
                  stories.classList.remove("filtered");
                } else {
                  gsap.set(stories, { display: "block" });
                  stories.classList.add("filtered");
        ...
        var showcaseListe = document.querySelector(".showcase-liste");

        if (hasFilteredItems) {
          showcaseListe.classList.add("items-filtered");
        } else {
          showcaseListe.classList.remove("items-filtered");
        }

        showcaseListe.classList.add("ease-transform");

        var endHeight = gsap.getProperty(".showcase-liste", "height");
        ...
        var flip = Flip.from(state, {
          ...
          onComplete: function () {
            ...
            showcaseListe.classList.remove("ease-transform");
          },
        });
      ...
      if (!isMobile()) {
        $(".showcase-liste .pax-item .slide-inner")
          .on("mouseenter", function () {
            $("#ball p").remove();
            var $this = $(this);
            gsap.to("#ball", {
        ...
        $(".showcase-liste .pax-item .slide-inner")
          .on("mouseenter", function () {
            if (!$(".showcase-liste").hasClass("list-grid")) {
      ...
      $(".trigger-item").on("click", function () {
        ...
        $(".showcase-liste .trigger-item").each(function () {
        ...
        gsap.to(
          "footer, .carousel-nav-wrapper, .showcase-liste.list-grid",
...
---------------------------------------------------*/
  ...
  function ShowcaseGallery() {
    if ($(".showcase-gallery").length > 0) {
      ...
      slider = new paxSlider(".showcase-gallery", {
        ...
        on: {
          init: function (slide) {
            if ($("body").hasClass("show-loader")) {
              imagesLoaded("body", function () {
                ...
                gsap.to(
                  $(".showcase-gallery .pax-slider .pax-slide .slide-inner"),
                  {
                    duration: 0.7,
                    opacity: 1,
                    delay: 0.4,
          ...
          slideLeaveViewport: function (slide) {
            gsap.set(
              $(".pax-slider div:not(.pax-slide-visible) .slide-events"),
              { x: "" }
            );
          },
        },
      ...
      const showGrid = () => {
        ...
        gsap.to(
          $(
            "footer .link-text, .pax-pagination, .progress-info, #filters-wrapper"
          ),
          {
            duration: 0.3,
            opacity: 0,
        ...
        gsap.set(currentImage.closest(".pax-slide"), { zIndex: 0 });
        ...
        gsap.to(".pax-slider .pax-slide .trigger-item", {
          duration: 1,
          opacity: 0,
          scale: 0.7,
          ease: Power2.easeOut,
        ...
        Flip.from(flipstate1, {
          ...
          onComplete: function () {
            ...
            const triggeredItem = document.querySelector(
              ".pax-slide.triggered-item"
            );
            if (triggeredItem != null) {
              triggeredItem.classList.remove("triggered-item");
            }

            const paxSlides = document.querySelectorAll(".pax-slide");
            paxSlides.forEach((slide) => {
              slide.style.zIndex = "";
      ...
      const getDOMElements = () => {
        ...
        const items = [];
        [...document.querySelectorAll(".pax-slide")].forEach((item) => {
          items.push(new Item(item));
        });

        // Cloned items
        const itemsCloned = [];
        [...document.querySelectorAll(".pax-slide-clone")].forEach((item) => {
        ...
        return {
          ...
          currentItem: new Item(
            document.querySelector(".pax-slide.triggered-item")
          ),
      ...
      const initEvents = () => {
        ...
        function moveThumbGrid(direction = "next") {
          if (document.body.classList.contains("grid-open")) {
            ...
            // Get all the slides
            let slides = document.querySelectorAll(".pax-slide");
            ...
            $(".pax-slide").removeClass("triggered-item");
      ...
      if (!isMobile()) {
        if ($(".showcase-gallery").hasClass("tilt-gallery")) {
          ...
          function callTiltSlider(e) {
            moveItSlider(e, ".pax-slider-viewport", 30);
          }
        ...
        $(".pax-slider").on("mouseup touchend", function () {
          gsap.to("#ball", {
            duration: 1,
            borderWidth: "4px",
            scale: 0.5,
        ...
        $(".showcase-gallery.preview-mode-enabled .pax-slide .slide-inner")
          .on("mouseenter", function () {
